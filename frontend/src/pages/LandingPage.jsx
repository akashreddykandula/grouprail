...
import {
  MdTrain, MdGroups, MdAutoAwesome, MdChat, MdNotifications, MdCheckCircle,
} from 'react-icons/md';
import {
  HiArrowRight, HiUserAdd, HiClipboardList, HiLightningBolt,
} from 'react-icons/hi';
  ...
  return (
    <div className="border-b border-white/10 last:border-0">
      ...
      >
        ...
        <span className={`shrink-0 w-6 h-6 rounded-full border border-white/20 flex items-center justify-center text-slate-400 transition-transform duration-300 ${open ? 'rotate-45' : ''}`}>
          +
        </span>
      </button>
      <motion.div
  ...
  return (
    <div className="overflow-x-hidden">
      ...
      <section id="features" className="py-24 border-t border-white/5">
        <div className="section-container">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
            {features.map((f, i) => (
              <motion.div key={f.title} {...fadeUp(i * 0.07)} className="glass-card-hover p-6">
      ...
      <section id="how-it-works" className="py-24 border-t border-white/5">
        <div className="section-container">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 relative">
            ...
            {steps.map((s, i) => (
              <motion.div key={s.number} {...fadeUp(i * 0.1)} className="relative glass-card p-6 text-center">
              ...
              <ul className="space-y-3">
                {[
                  ...
                  <li key={item} className="flex items-start gap-3 text-slate-300 text-sm">
                    <MdCheckCircle className="text-emerald-400 shrink-0 mt-0.5" size={18} />
                    {item}
                  </li>
                ))}
              </ul>
              ...
              <div className="glass-card p-6 border-primary-500/20">
                ...
                <div className="p-4 rounded-xl bg-primary-500/5 border border-primary-500/10 mb-4">
                  <p className="text-slate-200 text-sm leading-relaxed">
                    <span className="text-primary-300 font-medium">Board from Vijayawada (BZA)</span> instead of
                ...
                <div className="space-y-3">
                  <div>
                    <p className="text-emerald-400 text-xs font-medium mb-2 uppercase tracking-wide">Advantages</p>
                    <div className="space-y-1.5">
                      {['Originating station — more quota available', 'Train departs from here (no delay risk)', '3 of 5 members already board here'].map((a) => (
                        <div key={a} className="flex items-start gap-2 text-slate-300 text-xs">
                          <MdCheckCircle className="text-emerald-400 shrink-0 mt-0.5" size={13} />
                          {a}
                        </div>
                      ))}
                    </div>
                  ...
                  <div>
                    <p className="text-amber-400 text-xs font-medium mb-2 uppercase tracking-wide">Trade-offs</p>
        ...
        <div className="section-container">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {testimonials.map((t, i) => (
              <motion.div key={t.name} {...fadeUp(i * 0.1)} className="glass-card p-6">
                <StarRating count={t.rating} />
                <p className="text-slate-300 text-sm leading-relaxed my-4">"{t.text}"</p>
      ...
      <section id="faq" className="py-24 border-t border-white/5">
        <div className="section-container">
          <motion.div {...fadeUp()} className="text-center mb-16">
            <span className="badge bg-cyan-500/10 text-cyan-300 border border-cyan-500/20 mb-4">FAQ</span>
            <h2 className="text-3xl sm:text-4xl font-bold font-display mb-4">Common questions</h2>
          </motion.div>

          <motion.div {...fadeUp(0.1)} className="max-w-2xl mx-auto glass-card px-6 divide-y divide-white/0">
            {faqs.map((faq) => (
